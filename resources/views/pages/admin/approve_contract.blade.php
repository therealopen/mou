                        ...
                        <div class="card mg-b-30">
                           <div class="card-header">
                              <div class="d-flex justify-content-between align-items-center">
                                 <div>
                                    <h6 class="card-header-title tx-13 mb-0">Approve,Extend,Terminate Contract</h6>
                                 </div>
                              ...
                              <span style="color: rgb(0, 30, 255); font-weight: bold;">
                               {{ $contract->approval_status }} ({{ $contract->status_tpc }})
                                       ...
                                       <td>
                                          ...
                                          {{-- <button type="button" class="btn btn-info btn-sm" onclick="approveContract('{{ $contract->id }}', 'notapproved')">Approve</button> --}}
                                          <!-- Approve Button -->


                                          <button type="button" class="btn btn-info btn-sm" onclick="resendContract('{{ $contract->id }}', 'notapproved')">Resend</button>

                                       

                                        {{-- <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-edit"></i></button> --}}
...
{{-- resend contract --}}
<div class="modal fade" id="resendContractModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalVarying" aria-hidden="true">
   <div class="modal-dialog" role="document">
       <div class="modal-content">
           <div class="modal-header">
               <h5 class="modal-title" id="exampleModalVarying">Adding Comment (Reason for resend)</h5>
               <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                   <span aria-hidden="true">&times;</span>
               </button>
           </div>
           <div class="modal-body">
               <form method="POST" action="{{ route('processresendcontracts') }}">
                   @csrf
                   <div class="form-group">
                       <input type="hidden" id="contractId" name="contract_id" required>
                       <input type="hidden" id="contract_status" name="contract_status" value="notapproved" required>
...
<script>
function resendContract(contractId, status) {
   document.getElementById('contractId').value = contractId;
   document.getElementById('contract_status').value = status;
   $('#resendContractModal').modal('show');
}
</script>
...
<script>
   function approveContract(contractId,status) {
       document.getElementById('contractId').value = contractId; // Set the contractId value
       document.getElementById('contract_status').value = status;
       $('#approveContractModal').modal('show'); // Show the modal
   }
   </script>
